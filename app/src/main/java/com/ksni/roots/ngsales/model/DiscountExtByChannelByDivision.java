 ...
 */
public class DiscountExtByChannelByDivision {
    final static String TABLE = "sls_discount_ext_channel_division";
    ...
    public DiscountExtByChannelByDivision(SQLiteDatabase db){
        channel= "";
        division = "";
        discount = 0;
        this.db = db;
    ...
    public void delete(){
        db.delete(TABLE,  "channel=? AND division=? AND valid_from=? and valid_to=? and and id=?", new String[]{channel,division,valid_from,valid_to,String.valueOf(id)});
    }
    ...
    public boolean save(){
        try {
            ...
            if (isExist()) {
                ...
                db.update(TABLE,  cv,"channel=? AND division=? AND valid_from=? and valid_to=? and and id=?", new String[]{channel,division,valid_from,valid_to,String.valueOf(id)});
            }
            else{
                cv.put("channel", channel);
                cv.put("division", division);
                cv.put("period", valid_from);
