 ...
 */
public class DiscountSpcByChannelIndia {
    final static String TABLE = "sls_discount_spc_channel_india";
    ...
    public DiscountSpcByChannelIndia(SQLiteDatabase db){
        channel= "";
        zone= "";
        discount = 0;
        this.db = db;
    ...
    public void delete(){
        db.delete(TABLE,"channel=? and division=? and valid_from=? and valid_to=? and id=?", new String[]{channel,zone,valid_from,valid_to,String.valueOf(id)});
    }
    ...
    public boolean save(){
        try {
            ...
            if(isExist()) {
                ...
                db.update(TABLE, cv,"channel=? and division=? and valid_from=? and valid_to=? and id=?", new String[]{channel,zone,valid_from,valid_to,String.valueOf(id)});
            }else{
                cv.put("channel", channel);
                cv.put("division", zone);
                cv.put("discount", discount);
